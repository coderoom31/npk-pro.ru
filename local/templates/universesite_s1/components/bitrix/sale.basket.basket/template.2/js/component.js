	...
	BX.Sale.BasketComponent = {
		...
		toggleFilter: function(event)
		{
			...
			for (var i = 0; i < entities.length; i++)
			{
				...
				{
					BX.addClass(entities[i], 'intec-cl-border active');
				}
				else if (BX.hasClass(entities[i], 'active'))
				{
					BX.removeClass(entities[i], 'intec-cl-border active');
				}
			}
		...
		--REMOVED PRICE CHANGING BEFORE ITEM UPDATE TO GET RID OF PRICE BLINKING--
