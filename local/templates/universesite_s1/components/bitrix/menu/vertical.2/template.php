...
<!--<script type="text/javascript">
    template.load(function (data) {
        ...
        var root = data.nodes;
        var item = $(".menu-item", root);

        item.on('mouseover', function () {
            $(this).attr("data-selected", "true");
            $(this).children(".menu-item-items").stop().slideDown(500);
        }).on('mouseout', function () {
            $(this).attr("data-selected", "false");
            $(this).children(".menu-item-items").stop().slideUp(500);
        });
    }, {
        'name': '[Component] bitrix:menu (vertical.2)',
        'nodes': <?php //echo JavaScript::toObject('#'.$sTemplateId) ?>,
        'loader': {
            'name': 'lazy'
        }
    });
