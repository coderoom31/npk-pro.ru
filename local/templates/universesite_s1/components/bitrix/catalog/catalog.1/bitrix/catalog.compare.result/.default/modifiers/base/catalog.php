...
foreach ($arResult['ITEMS'] as &$arItem) {
    ...
    if (!empty($arItem['PRICE_MATRIX']) && !empty($arItem['PRICE_MATRIX']['MATRIX'])) {
        foreach ($arItem['PRICE_MATRIX']['MATRIX'] as $iPriceTypeId => $arPrices) {
            ...
            if (empty($arItem['MIN_PRICE']) || $arItem['MIN_PRICE']['DISCOUNT_VALUE'] > $arPrice['DISCOUNT_PRICE']) {
                $arItem['MIN_PRICE'] = [
                    'TYPE' => $iPriceTypeId,
    ...
    } else if (!empty($arItem['PRICES'])) {
        foreach ($arItem['PRICES'] as $arPrice) {
            if (empty($arItem['MIN_PRICE']) || $arItem['MIN_PRICE']['DISCOUNT_VALUE'] > $arPrice['DISCOUNT_VALUE_VAT']) {
                $arItem['MIN_PRICE'] = [
                    'TYPE' => Type::toInteger($arPrice['PRICE_ID']),
                    'CURRENCY' => $arPrice['CURRENCY'],
                    'VAT_RATE' => Type::toFloat($arPrice['VATRATE_VALUE']),
                    'VALUE' => Type::toFloat($arPrice['VALUE_VAT']),
                    'PRINT_VALUE' => $arPrice['PRINT_VALUE_VAT'],
                    'DISCOUNT_VALUE' => Type::toFloat($arPrice['DISCOUNT_VALUE_VAT']),
                    'PRINT_DISCOUNT_VALUE' => $arPrice['PRINT_DISCOUNT_VALUE_VAT']
                ];
            }
        }
    }
