...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		getErrorContainer: function(node)
		{
			...
			/* CUSTOM */
            node.appendChild(
                BX.create('DIV', {props: {className: 'intec-ui intec-ui-control-alert intec-ui-scheme-red intec-ui-m-b-20'}, style: {display: 'none'}})
            );
		},
		...
		showError: function(node, msg, border)
		{
			...
			/* CUSTOM */
            var errorContainer = node.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-red'), animate;
		...
		showErrors: function(errors, scroll, showAll)
		{
			...
			/* CUSTOM */
            var errorNodes = this.orderBlockNode.querySelectorAll('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
		...
		showBlockErrors: function(node)
		{
			...
			/* CUSTOM */
            var errorNode = node.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
		...
		checkNotifications: function()
		{
			...
			if (informer)
			{
				if (this.firstLoad && this.result.IS_AUTHORIZED && typeof this.result.LAST_ORDER_DATA.FAIL === 'undefined')
				{
                    ...
                    BX.addClass(informer, 'intec-ui intec-ui-control-alert intec-ui-scheme-' + className + ' intec-ui-m-b-20');
				...
				/* CUSTOM */
				else if (BX.hasClass(informer, 'intec-ui'))
				{
		...
		checkBlockErrors: function(node)
		{
			...
			if (hiddenNode = BX(node.id + '-hidden'))
			{
				...
				/* CUSTOM */
                errorNode = hiddenNode.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
                showWarning = hiddenNode.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange.alert-show');
		...
		scrollToError: function()
		{
			...
			for (i in sections)
			{
				if (sections.hasOwnProperty(i))
				{
					...
					/* CUSTOM */
                    errorNode = sections[i].querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
		...
		showWarnings: function()
		{
			...
			if (currentDelivery && currentDelivery.CALCULATE_ERRORS)
			{
				...
				if (this.activeSectionId != this.deliveryBlockNode.id)
				{
					...
					/* CUSTOM */
                    BX.bind(this.deliveryBlockNode.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange'), 'click', BX.proxy(this.showByClick, this));
				}
			}
		...
		showBlockWarning: function(node, warnings, hide)
		{
			...
			/* CUSTOM */
            var errorNode = node.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
			...
			if (errorNode)
			{
				...
				/* CUSTOM */
				existedWarningNodes = node.querySelectorAll('.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange');
				...
				warningNode = BX.create('DIV', {
					...
					/* CUSTOM */
                    props: {className: 'intec-ui intec-ui-control-alert intec-ui-scheme-orange intec-ui-m-b-20' + (!!hide ? ' alert-hide' : ' alert-show')},
		...
		markSectionAsCompleted: function(section)
		{
			...
			/* CUSTOM */
			BX.remove(section.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange.alert-hide'));
		...
		fade: function(node, nextSection)
		{
			...
			switch (node.id)
			{
				...
				case this.paySystemBlockNode.id:
					...
					/* CUSTOM */
					BX.remove(this.paySystemBlockNode.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange.alert-hide'));
				...
				case this.deliveryBlockNode.id:
					...
					/* CUSTOM */
					BX.remove(this.deliveryBlockNode.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange.alert-hide'));
		...
		editSection: function(section)
		{
			...
			/* CUSTOM */
            errorContainer = section.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
		...
		editAuthBlock: function()
		{
            ...
            /* CUSTOM START */
            var authRow = BX.create('DIV', {props: {className: 'bx-row'}});
            var regRow = BX.create('DIV', {props: {className: 'bx-row'}});
			...
			if (this.result.OK_MESSAGE && this.result.OK_MESSAGE.length)
			{
				...
				okMessageNode = BX.create('DIV', {
					...
					/* CUSTOM */
                    props: {className: 'intec-ui intec-ui-control-alert intec-ui-scheme-green intec-ui-m-b-20'},
		...
		editAuthorizeForm: function(authContent)
		{
            ...
            remember = BX.create('DIV', {
                ...
                children: [
                    BX.create('LABEL', {
                        props: {className: 'intec-ui intec-ui-control-checkbox intec-ui-scheme-current'},
                        children: [
                            BX.create('INPUT', {
                                props: {
                                    type: 'checkbox',
                            ...
                            BX.create('SPAN', {props: {className: 'intec-ui-part-selector'}}),
                            BX.create('SPAN', {props: {className: 'intec-ui-part-content'}, text: BX.message('STOF_REMEMBER')})
                        ]
                    })
                ]
            });
			...
			button = BX.create('DIV', {
				...
				children: [
                    ...
                    BX.create('INPUT', {
                        props: {
                            type: 'submit',
                            className: 'intec-ui intec-ui-control-button intec-ui-mod-round-3 intec-ui-scheme-current intec-ui-size-2',
			...
			authContent.appendChild(BX.create('DIV', {props: {className: 'bx-authform-wrapper'}, children: [authFormNode]}));
		},
		...
		editRegistrationForm: function(authContent)
		{
            ...
            else
			{
                ...
                if (this.authGenerateUser) {
                    ...
                    radioNode.appendChild(
                        ...
                        BX.create('LABEL', {
                            props: {
                                className: 'intec-ui intec-ui-control-radiobox intec-ui-scheme-current',
                                for: 'NEW_GENERATE_N'
                            },
                            children: [
                                BX.create('INPUT', {
                                ...
                                BX.create('SPAN', {props: {className: 'intec-ui-part-selector'}}),
                                BX.create('SPAN', {
                                    props: {className: 'intec-ui-part-content'},
                                    text: BX.message('STOF_MY_PASSWORD')
                                })
                            ],
                            events: {
                    ...
                    radioNode.appendChild(
                        ...
                        BX.create('LABEL', {
                            ...
                            children: [
                                ...
                                BX.create('SPAN', {
                                    props: {className: 'intec-ui-part-content'},
                                    text: BX.message('STOF_SYS_PASSWORD')
                                })
                            ],
                            events: {
                                change: BX.delegate(function () {
                    ...
                    BX.create('DIV', {
                        ...
                        children: [
                            ...
                            BX.create('INPUT', {
                                props: {
                                    type: 'submit',
                                    className: 'bx-authform-buttons-register intec-ui intec-ui-control-button intec-ui-mod-round-3 intec-ui-scheme-current intec-ui-size-2',
			...
			authContent.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-authform-wrapper'},
		...
		editSocialContent: function(authContent)
		{
			...
			if (this.result.AUTH.new_user_registration === 'Y')
			{
				nodes.push(BX.create('DIV', {
					...
					children: [
						...
						/* CUSTOM */
                        BX.create('A', {
                            props: {className: 'intec-ui intec-ui-control-button intec-ui-mod-round-3 intec-ui-scheme-current intec-ui-size-2'},
			...
			authContent.appendChild(BX.create('DIV', {props: {className: 'bx-authform-wrapper'}, children: nodes}));
		},
		...
		getAuthReference: function(authContent)
		{
			authContent.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-authreference-wrapper'},
					children: [
						BX.create('DIV', {
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				regionNode = BX.create('DIV', {props: {className: 'intec-grid intec-grid-wrap intec-grid-i-h-15 bx_soa_location'}});
				regionNodeCol = BX.create('DIV', {props: {className: 'intec-grid-item bx_soa_location-col'}});
		...
		editFadeRegionContent: function(node)
		{
			...
			/* CUSTOM */
            var errorNode = this.regionHiddenBlockNode.querySelector('.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
			...
			if (altProperty)
			{
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'intec-ui intec-ui-control-input intec-ui-mod-block form-control bx-soa-customer-input bx-ios-fix',
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						currentType = this.result.PERSON_TYPE[i];
						label = BX.create('LABEL', {
							props: {className: 'intec-ui intec-ui-control-radiobox intec-ui-scheme-current intec-ui-size-2'},
							children: [
								BX.create('INPUT', {
                                ...
                                BX.create('SPAN', {
                                    props: {className: 'intec-ui-part-selector'}
                                }),
                                BX.create('SPAN', {
                                    props: {className: 'intec-ui-part-content'},
                                    html: BX.util.htmlspecialchars(currentType.NAME)
                                })
							],
							events: {change: BX.proxy(this.sendRequest, this)}
						});
		...
		createPaySystemItem: function(item)
		{
            ...
            label = BX.create('LABEL', {
                ...
                children: [
                    ...
                    BX.create('SPAN', {
                        props: {
                            className: 'intec-ui-part-content'
                        },
                        text: paySystemText
                    })
                ]
		...
		editFadePaySystemContent: function(node)
		{
            ...
            /* CUSTOM */
            var errorNode = this.paySystemHiddenBlockNode.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
            var warningNode = this.paySystemHiddenBlockNode.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange.alert-show');
		...
		selectPaySystem: function(event)
		{
			...
			else if (actionSection)
			{
				if (BX.hasClass(actionSection, 'bx-selected')) {
                    if (event.toElement && event.toElement.tagName === 'A')
                        return;
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var	currentDelivery, logotype, name, logoNode,
				...
				clear, infoList, extraServices, extraServicesNode, extraDesc;
			...
			if (currentDelivery.CALCULATE_DESCRIPTION) {
				extraDesc = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-block'},
            ...
            deliveryInfoContainer.appendChild(
                BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company'},
                    children: [clear, infoList, extraServicesNode, extraDesc]
                })
            );
		...
		createDeliveryItem: function(item)
		{
            ...
            if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y') {
                ...
                title = this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME;
		...
		editFadeDeliveryContent: function(node)
		{
			...
			/* CUSTOM */
            var errorNode = this.deliveryHiddenBlockNode.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-red');
            var warningNode = this.deliveryHiddenBlockNode.querySelector('div.intec-ui.intec-ui-control-alert.intec-ui-scheme-orange.alert-show');
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				pickUpContentCol = BX.create('DIV', {props: {className: 'bx_soa_pickup-col'}});
		...
		editPickUpList: function(isNew)
		{
			...
			if (!!isNew)
			{
				container = this.pickUpHiddenBlockNode.querySelector('.bx_soa_pickup>.bx_soa_pickup-col');
				if (!container)
					container = this.pickUpBlockNode.querySelector('.bx_soa_pickup>.bx_soa_pickup-col');
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							/* CUSTOM */
                            BX.create('A', {
                                props: {href: '', className: 'intec-ui intec-ui-control-button intec-ui-scheme-current'},
		...
		selectStore: function(event)
		{
			...
			if (storeItem && storeInput)
			{
				...
				button = BX.findChild(selectedPickUp, {className: 'intec-ui-control-button'}, true);

				if (button)
					button.text = this.params.MESS_SELECT_PICKUP;

				button = BX.findChild(storeItem, {className: 'intec-ui-control-button'}, true);
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				propsNode = BX.create('DIV', {props: {className: 'intec-grid intec-grid-wrap intec-grid-i-h-15 bx-soa-section-row'}});
				selectedDelivery = this.getSelectedDelivery();
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'intec-grid-item-1 intec-grid intec-grid-wrap bx-soa-customer'}}),
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "intec-grid-item-1 form-group bx-soa-customer-field");
					...
					if (property.isMultiple())
					{
						propContainer.appendChild(
							BX.create('DIV', {
								...
								props: {className: 'intec-ui intec-ui-control-button intec-ui-scheme-current'},
		...
		insertYNProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				/* CUSTOM */
                propContainer = BX.create('DIV', {props: {className: 'soa-property-container'}});
                propLabel = BX.create('LABEL', {props: {className: 'intec-ui intec-ui-control-checkbox intec-ui-scheme-current'}});
                property.appendTo(propLabel);
                propLabel.appendChild(BX.create('SPAN', {props: {className: 'intec-ui-part-selector'}}));
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'intec-ui intec-ui-control-input intec-ui-mod-block form-control bx-soa-customer-input bx-ios-fix');
			}
			...
			{
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], 'intec-ui intec-ui-control-input intec-ui-mod-block form-control bx-ios-fix');
			}
			...
			{
				BX.addClass(add[i], 'intec-ui intec-ui-control-button intec-ui-scheme-current');
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					...
					if (add.length && add[add.length - 2])
					{
						...
						BX.addClass(add[add.length - 2], 'intec-ui intec-ui-control-button intec-ui-scheme-current');
					}
					...
					if (textInputs.length)
					{
						...
						BX.addClass(textInputs[textInputs.length - 1], 'intec-ui intec-ui-control-input intec-ui-mod-block form-control bx-soa-customer-input bx-ios-fix');
		...
		{
			var propsMapContainer = BX.create('DIV', {props: {className: 'props-map-container'}, style: {marginBottom: '10px'}}),
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'intec-grid-item-1 props-comment-container'}});
			label = BX.create('LABEL', {
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'intec-ui intec-ui-control-input intec-ui-mod-block form-control bx-soa-customer-textarea bx-ios-fix',
		...
		editTotalBlock: function()
		{
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-cart-total-button-container ' + (showOrderButton ? ' visible' : 'hidden')},
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'intec-ui intec-ui-control-button intec-ui-mod-block intec-ui-mod-round-3 intec-ui-scheme-current intec-ui-size-2 btn-order-save'
								},
