...
if(!empty($arCurrentValues['PROPERTY_PRICE'])){
    $arTemplateParameters['PRICE_OLD_SHOW'] = [
        'PARENT' => 'VISUAL',
        'NAME' => Loc::getMessage('C_CATALOG_SECTION_SERVICES_SMALL_1_PRICE_OLD_SHOW'),
        'TYPE' => 'CHECKBOX',
        'DEFAULT' => 'N'
    ];
    if($arCurrentValues['PRICE_OLD_SHOW'] === 'Y'){
        $arTemplateParameters['PROPERTY_PRICE_OLD'] = [
            'PARENT' => 'DATA_SOURCE',
            'NAME' => Loc::getMessage('C_CATALOG_SECTION_SERVICES_SMALL_1_PROPERTY_PRICE_OLD'),
            'TYPE' => 'LIST',
            'VALUES' => $arProperties->asArray($hPropertiesString)
        ];
