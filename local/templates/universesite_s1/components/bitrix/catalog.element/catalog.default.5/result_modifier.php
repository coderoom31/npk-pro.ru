...
$arParams = ArrayHelper::merge([
    ...
    'SHARES_SHOW' => 'N',
    'SHARES_ONLY_ACTIVE_DATE' => 'N',
    'PROPERTY_SHARES' =>null,
    'SIZES_SHOW' => 'N',
    'SIZES_PATH' => null,
    'SIZES_MODE' => 'all',
...
if ($arVisual['SHARES']['SHOW'] && !empty($arParams['PROPERTY_SHARES'])) {

    if (empty($arVisual['SHARES']['HEADER']))
        $arVisual['SHARES']['HEADER'] = Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_SHARES_HEADER_DEFAULT');

    $arShares = $arResult['PROPERTIES'][$arParams['PROPERTY_SHARES']];
    $arResult['SHARES'] = [
        'SHOW' => $arParams['SHARES_SHOW'] === 'Y',
        'IBLOCK_ID' => !empty($arParams['SHARES_IBLOCK_ID']) ? $arParams['SHARES_IBLOCK_ID'] : null,
        'ITEMS' => []
    ];

    if ($arParams['SHARES_ONLY_ACTIVE_DATE'] === 'Y') {
        $arFilterShares = [
            'ACTIVE_DATE' => 'Y',
            'ID' => $arShares['VALUE']
        ];
    ...
    $arResult['SHARES']['ITEMS'] = Arrays::fromDBResult(CIBlockElement::GetList(
        $arOrderShares,
        $arFilterShares
    ))->indexBy('ID')->asArray();
