...
$arParams = ArrayHelper::merge([
    ...
    'SELECTOR' => null,
    '~SELECTOR' => null,
    'MOBILE_PICTURE_USE' => 'N',
    'PROPERTY_MOBILE_PICTURE' => null,
], $arParams);
...
$arVisual = [
    ...
    'MOBILE' => [
        'PICTURE' => [
            'USE' => $arParams['MOBILE_PICTURE_USE'] === 'Y' && !empty($arParams['PROPERTY_MOBILE_PICTURE'])
        ]
    ],
    'VIDEO' => [
...
foreach ($arResult['ITEMS'] as &$arItem) {
    ...
    if (!empty($arParams['PROPERTY_MOBILE_PICTURE'])) {

        $arProperty = ArrayHelper::getValue($arItem, [
            'PROPERTIES',
            $arParams['PROPERTY_MOBILE_PICTURE'],
            'VALUE'
        ]);
        ...
        if (!empty($arProperty)) {
            ...
            if (!empty($arProperty) && !$arPictures->has($arProperty)) {
                ...
                $arItem['DATA']['MOBILE']['PICTURE']['VALUE'] = $arProperty;
                $arItem['DATA']['MOBILE']['PICTURE']['USE'] = $arResult['VISUAL']['MOBILE']['PICTURE']['USE'];
            }
        }
    }
...
if (!$arFiles->isEmpty() || !$arPictures->isEmpty()) {
    foreach ($arResult['ITEMS'] as &$arItem) {
        ...
        if (!$arPictures->isEmpty()) {
            ...
            $arItemMobilePicture = $arItem['DATA']['MOBILE']['PICTURE']['VALUE'];

            if (!empty($arItemMobilePicture) && $arPictures->exists($arItemMobilePicture))
                $arItem['DATA']['MOBILE']['PICTURE']['VALUE'] = $arPictures->get($arItemMobilePicture);

            unset($arItemMobilePicture);
        }
    }
}
